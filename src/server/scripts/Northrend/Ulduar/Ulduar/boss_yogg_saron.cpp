...
class boss_voice_of_yogg_saron : public CreatureScript
{
    public:
        ...
        {
			boss_voice_of_yogg_saronAI(Creature* creature) : BossAI(creature, NPC_YOGG_SARON)
            {
            ...
            void JustDied(Unit* killer) override
            {
				...
				if (Creature* yogg = ObjectAccessor::GetCreature(*me, instance->GetData64(NPC_YOGG_SARON)))
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_LOCK_DOOR:
							...
							instance->SetBossState(NPC_YOGG_SARON, IN_PROGRESS);
                            break;
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_TRANSFORM_4:
							...
							if (Creature* yogg = ObjectAccessor::GetCreature(*me, _instance->GetData64(NPC_YOGG_SARON)))
...
class spell_yogg_saron_empowered : public SpellScriptLoader     // 64161
{
    public:
        ...
        class spell_yogg_saron_empowered_AuraScript : public AuraScript
        {
            ...
            void OnApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
            {
                GetTarget()->CastCustomSpell(SPELL_EMPOWERED_BUFF, SPELLVALUE_AURA_STACK, 9, GetTarget(), false);
            }
            ...
            void OnPeriodic(AuraEffect const* /*aurEff*/)
            {
                ...
                if (stack)
                {
                    ...
                    target->CastCustomSpell(SPELL_EMPOWERED_BUFF, SPELLVALUE_AURA_STACK, stack, target, false);
                }
...
class spell_yogg_saron_in_the_maws_of_the_old_god : public SpellScriptLoader    // 64184
{
    public:
        ...
        class spell_yogg_saron_in_the_maws_of_the_old_god_SpellScript : public SpellScript
        {
            ...
            {
                if (InstanceScript* instance = GetCaster()->GetInstanceScript())
					if (Creature* yogg = ObjectAccessor::GetCreature(*GetCaster(), instance->GetData64(NPC_YOGG_SARON)))
