...
public:
    ...
    {
        boss_grand_vizier_ertanAI(Creature* creature) : BossAI(creature, NPC_GRAND_VIZIER_ERTAN) { }
        ...
        void JustDied(Unit* /*who*/)
        {
			...
			Slipstream->SetUInt32Value(UNIT_FIELD_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
        }
    ...
    struct npc_cyclone_shieldAI : public ScriptedAI
    {
        ...
        void UpdateAI(uint32 const diff)
        {
            ...
            if (events.ExecuteEvent())
            {
                ...
                TC_LOG_ERROR("misc", "position:%u guid:%u", position, me->GetGUID());
            }
        }
...
public:
    ...
    struct npc_lurking_tempestAI : public ScriptedAI
    {
        npc_lurking_tempestAI(Creature* creature) : ScriptedAI(creature)
        {
            ...
            if (Unit* Boss = me->FindNearestCreature(NPC_GRAND_VIZIER_ERTAN, 100.0f))
            {
